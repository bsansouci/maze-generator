// Version 5
// Uses random boundaries for primary/secondary room generation
...
function setRowsTo(space, height, bool) {
  for (var i = 0; i < slices; i++) {
    var p = map(space, function(val) {
      val[height * i].reusable = bool;
    });
  }
}

function splice(arr, f) {
...
function addRegionsToGraph(space, mazeGraph) {
  mazeGraph.primaryRooms[0].region = {
    x: 0,
    y: 0,
    width: rand(space.length - 5),
    height: rand(space[0].length - 5)
  };
...
function generateMaze() {
  ...
  addRandomPaths(mazeGraph, space, 0);
...
if(!maze) {
  var r = randFrom([ 'qrkshidoelmp', 'nocqajsihgrlpkme', 'ljkhpdnmt', 'bslqktdripjnmhoa', 'gkjdl', 'lrmkojianfhtpg', 'rehfog', 'kaneritboqldcpghjs', 'cofge', 'chlkqerdf', 'bahsnc', 'ekpmglfbacithjsn', 'mgajhsknftloqerd', 'ifeg', 'qfhklndticmj', 'ldjbntafi', 'esdqlkhbotp', 'firlhmqjok', 'tpesoihnfjbqd', 'hmakgnspjdtieblc', 'rqoahflibkdp', 'jmtncalrokdgiq', 'cfketpnr', 'dbcasnpghfiokjm', 'nrcfiqmshbekdatlo' ]);
