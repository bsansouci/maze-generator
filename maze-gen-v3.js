...
function addRandomPaths(maze) {
  blockRoom(maze.mazeGraph.startNode.room);
  blockRoom(maze.mazeGraph.exitNode.room);
  maze.mazeGraph.primaryRooms.map(function(x){blockRoom(x.room);});
  maze.mazeGraph.secondaryRooms.map(function(x){blockRoom(x.room);});

  var space = maze.space;
  var possibleCells = shuffle(flatten(space).filter(function(x) {return x.reusable;}));

  var totalTries = rand(200, 20);
  while(totalTries--) {
