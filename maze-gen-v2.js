...
function addRandomPaths(maze) {
  var space = maze.space;
  var possibleCells = shuffle(flatten(space).filter(function(x) {return x.reusable;}));

  var rooms = shuffle([maze.mazeGraph.startNode].concat(maze.mazeGraph.primaryRooms).filter(function(x) {
    return x.adjacencyList.length < 4;
  }));

  if(rooms.length < 2) {
    console.log("Can't add more paths because there are no two rooms that we can connect together");
    return;
  }

  var startNode = rooms.pop();
  var endNode = rooms.pop();

  startNode.room.cells[1][1].reusable = true;
  var prev = startNode.room.cells[1][1];

  var numOfIntermediatPoints = rand(4, 1);
  var intermediatePoints = [];
  while(numOfIntermediatPoints) {
    if(possibleCells.length === 0) {
      console.log("couldn't link any of those points...");
      return;
    }
    var cell = possibleCells.pop();
    var p = findPath(space, prev, cell);
    if(p.length > 0) {
      intermediatePoints.push(p);
      prev = cell;
      --numOfIntermediatPoints;
    }
  }

  endNode.room.cells[1][1].reusable = true;
  var p = findPath(space, prev, endNode.room.cells[1][1]);
  if(p.length === 0){
    intermediatePoints.map(digBlockingPath);
    console.log("Can't connect last point to endNode. Added", intermediatePoints.length - 1, "points");
    return;
  }
  intermediatePoints.push(p);
  ...
  // Dig all paths
  intermediatePoints.map(digBlockingPath);
  console.log("Added", intermediatePoints.length, "paths");
}
